...
setwd("/home/damien/Downloads/Practical")
```
...
The resulting alignment is stored as a `DNAbin` object. Printing this object gives us some information about the alignemnt.
...
**Question:** How many nucleotides do you expect?
</span>
<details>
...
<span style="background-color:#EFD31C">
**Question:** How many samples are variant at 23404 (reference `a`)?
</span>
<details>
  <summary>**Show answer**</summary>
...
Lets take a look at the continents represented. We can do this using the `$` sign and the `table()` command as follows:  
...
However at this point in the pandemic there were plenty of sampling gaps, for example no genomes available from Africa.
   
Lets look at another variable, for example the isolation sources again using the `$`sign and the `table()` command to find total numbers in our data:
  
```{r explore_meta_2}
table(meta$Specimen)
```  

There are lots of sample sources with most from alveolar lavage fluid. However scroll down the list - the metadata is incomplete and not listed consistently. 
...
R efficiently stores this tree as a `phylo format` object. You can this tree has 799 tips and each tip corresponds to a genome in our dataset. We can look at the order of tips by extracting variables from this object. For example to print the first ten tip labels:
...
<span style="background-color:#EFD31C">
**Question:** Even at this early stage of the pandemic there is not much in the way of geographic structure. What does this mean?
</span>
<details>
  <summary>**Show answer**</summary>
...
```{r date_3}
date.dec.vec <- decimal_date(as.Date(date.vec,'%d/%m/%Y'))
...
<span style="background-color:#EFD31C">
**Question:** The p-value of the temporal regression is highly significant. What does this mean?
</span>
<details>
  <summary>**Show answer**</summary>
...
<span style="background-color:#EFD31C">
**Question:** Why might our inferred date be too recent?
</span>
<details>
  <summary>**Show answer**</summary>
...
![**NextStrain dated phylogeny**](data/nextstrain_tree.png){#id .class width=80% height=80%}

# Conclusions

Hopefully this practical has served as an introduction to the flexibility of R for analysing phylogenetic data. This analysis demonstrates the feasibility of estimating the age of SARS-CoV-2 using a small dataset encompassing the genomes uploaded during the first three months of the COVID-19 genomics sequencing efforts.
...
![**Global SARS-CoV-2 datasets reached 100 thousand sequences within only nine months of the start of the pandemic**](data/global_tree.png){#id .class width=70% height=70%}
